...
A lot of this (particularely for *Spotify* and *Genius*) comes from a nice post by [*RCharlie*](http://www.rcharlie.com/post/fitter-happier/). *RCharlie* is also the author of some `R` packages we'll be using: *sentify* and [*spotifyr*](https://cran.r-project.org/package=spotifyr).

## *Spotify*

First and foremost we should note that *Spotify* no longer supports location queries with its API.  Doing a little digging about how that could be done I found [this](https://stackoverflow.com/questions/11367264/solutions-for-finding-website-visitors-geolocation) and [this](http://freegeoip.net/?q=192.12.12.251).  I'll come back to that.

But before getting into the complex stuff, we can still use the *Spotify* API as is to access a metric of popularity, connections between songs, and the sonic "mood" of a song---its valance. We can most easily do all that with the `R` package *spotifyr*, which wraps the API. Install it through CRAN:
```{r, eval=FALSE}
install.packages('spotifyr') # also installs the many dependencies
```

Before using the API, through *spotifyr* or directly, we need to setup a dev account with *Spotify*. To do so follow the instructions [here](https://developer.spotify.com/web-api/tutorial/), starting at "Setting Up Your Account" (follow the re-direction link).
